...
MSAA::MSAA(DXGI_FORMAT backBufferFormat,
...
void MSAA::SetDevice(ID3D11Device* device)
{
...
void MSAA::SizeResources(size_t width, size_t height)
{
...
void MSAA::Resolve(ID3D11DeviceContext* context, ID3D11Texture2D* backBuffer)
{
    context->ResolveSubresource(backBuffer, 0, m_msaaRenderTarget.Get(), 0, m_backBufferFormat);
}


void MSAA::SetWindow(const RECT& output)
{
